...
function Indent() {
  ...
  useEffect(() => {
    ...
    // Reusable function to initialize our real-time indent channel subscription safely
    const startIndentSubscription = () => {
      console.log("Initializing real-time indent stream...");
      connectNotification(
        Number(currentUser.id),
        () => {},
        (indent) => {
    ...
    // 1. Try subscribing immediately on page render
    startIndentSubscription();

    // 2. Listen for MainLayout's post-login success event to catch the connection if it was delayed
    window.addEventListener("socket-connected", startIndentSubscription);

    return () => {
      window.removeEventListener("online-users-updated", handleOnlineUsersUpdate);
      window.removeEventListener("socket-connected", startIndentSubscription);
    };
  }, [currentUser.id]);
